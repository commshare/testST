...
void _st_vp_schedule(void)
{
  ...
  _ST_RESTORE_CONTEXT(thread); //÷¥––µΩ’‚¿Ô£¨æÕÃ¯◊™µΩ¡À617––ƒ«¿Ô£¨Œ™…∂ƒÿ?
}
...
void st_thread_exit(void *retval)
{
  ...
  if (!(thread->flags & _ST_FL_PRIMORDIAL)) { LOGD("#####VALGRIND#####");
    VALGRIND_STACK_DEREGISTER(thread->stack->valgrind_stack_id); //’‚¿Ô“™◊ﬂÂ
  }
#endif

  if (!(thread->flags & _ST_FL_PRIMORDIAL)) {
  	LOGD("#####_st_stack_free#####");
    _st_stack_free(thread->stack); //◊ﬂ’‚¿Ô
  }

  	LOGD("#####Find another thread to run#####");
  /* Find another thread to run ∑¢œ÷¡ÌÕ‚“ª∏ˆœﬂ≥Ã“≤‘⁄‘À––*/
  _ST_SWITCH_CONTEXT(thread);
   	LOGD("#####  free(thread);#####");
  free(thread);
...
void _st_thread_main(void)
{
  ...
  /* Run thread main ÷¥––œﬂ≥Ãµƒ∫Ø ˝£¨do_resolveæÕ «“ª∏ˆ*/
  thread->retval = (*thread->start)(thread->arg);

  //µΩ¡À’‚¿Ô£¨æÕÀµ…œ√Êµƒœﬂ≥Ã∫Ø ˝÷¥––ÕÍ±œ¡À°£
...
#if defined (MD_STACK_GROWS_DOWN) /*’‚∏ˆ÷¥––*/
  sp = stack->stk_top;
#ifdef __ia64__
  /*
...
#elif defined (MD_STACK_GROWS_UP) //’‚¿Ô≤ª◊ﬂ
  sp = stack->stk_bottom;
  thread = (_st_thread_t *) sp;
  sp = sp + sizeof(_st_thread_t);
  ptds = (void **) sp;
...
#ifndef __ia64__
   /*ª·◊ﬂ’‚¿Ô*/ //√≤À∆’‚¿Ô“≤≤ª◊ﬂ∞° 20150922£¨∫Û¿¥Ã¯◊™µΩ¡À’‚¿Ô£¨÷¥––’‚∏ˆmain
...
void _st_iterate_threads(void) /*¥”290Ã¯◊™µΩ663*/
{
  ...
  if (thread == _ST_CURRENT_THREAD()) //◊ﬂ’‚¿Ô
    MD_LONGJMP(orig_jb, 1);
